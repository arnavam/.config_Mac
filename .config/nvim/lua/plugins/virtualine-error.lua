return {
    ...
    priority = 1000,
    config = function()
-- Disable Neovim's built-in diagnostic virtual text
vim.diagnostic.config({ virtual_text = false })

-- Filter out specific diagnostics from being published at all
vim.lsp.handlers["textDocument/publishDiagnostics"] = vim.lsp.with(
  function(_, result, ctx, config)
    if result.diagnostics then
      result.diagnostics = vim.tbl_filter(function(d)
        local msg = d.message
        return not (msg:match("W191") or msg:match("indentation contains tabs") or msg:match("E501") or msg:match("line too long") or msg:match("imported but unused"))
...
require("tiny-inline-diagnostic").setup({
    ...
    options = {
        ...
        add_messages = {
            messages = false,           -- Show messages only on cursor line
            display_count = false,      -- Show count on other lines
        ...
        multilines = {
            enabled = true,            -- Required for display_count to work
            always_show = false,       -- Only show on cursor line
